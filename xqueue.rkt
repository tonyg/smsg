...
(define (pushback! q v)
  (let ((new (link v (queue-head q))))
    (when (not (queue-head q))
      (set-queue-tail! q new))
    (set-queue-head! q new)))
...
(define (queue-append! q1 q2)
  (when (queue-head q2)
    (if (queue-head q1)
        (set-link-tail! (queue-tail q1) (queue-head q2))
        (set-queue-head! q1 (queue-head q2)))
    (set-queue-tail! q1 (queue-tail q2))
    (set-queue-head! q2 #f)
    (set-queue-tail! q2 #f)))
...
(provide/contract
 ...
 [queue-append! (-> queue/c queue/c void?)])

(provide enqueue! dequeue! pushback!)
